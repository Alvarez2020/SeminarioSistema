<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Unidad_Medida.aspx.cs" Inherits="SistemaCONNY.CatalogosSistema.Unidad_Medida" %>
                ...
                <div class="modal-body">
                    ...
                    <div class="panel-body">
                        ...
                        <div hidden class="form-group">
                            <asp:Label Text="ID_UNIDAD_MEDIDA" runat="server" />
                            <asp:TextBox ID="IdUnidadMedida" runat="server" class="form-control" type="text" Text="0" placeholder="ID_UNIDAD_MEDIDA"></asp:TextBox>
                        </div>
                        <div class="form-group">
                            <asp:Label Text="UM_DESCIPCION" runat="server" />
                            <asp:TextBox ID="txtUmDescripcion" runat="server" CssClass="form-control" type="text" placeholder="UM_DESCRIPCION"></asp:TextBox>
                            <asp:RequiredFieldValidator ForeColor="red" ValidationGroup="valGuardar" ControlToValidate="txtUmDescripcion" ID="RequiredFieldValidator1" runat="server" ErrorMessage="Campo es necesario" />

                        </div>
                        <div class="form-group">
                            <asp:Label Text="UNIDADADES" runat="server" />
                            <asp:TextBox ID="txtUnidades" runat="server" CssClass="form-control" TextMode="Number" type="text" placeholder="UNIDADES"></asp:TextBox>
                            <asp:RequiredFieldValidator ForeColor="red" ValidationGroup="valGuardar" ControlToValidate="txtUnidades" ID="RequiredFieldValidator2" runat="server" ErrorMessage="Campo es necesario" />
                       </div>
                    </div>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderScript" runat="server">
     <script>    
         ...
         function LoadTemp() {
             ...
             $.ajax({
                 type: "POST",
                 url: "<%= ResolveUrl("Unidad_Medida.aspx/CargarDatos") %>",
            ...
            //funcion que permite seleccionar datos de la tabla
             $('#gridData').on('click', '#btnEditar', function () {
                ...
                // console.log(globalDataEdicion);
                 var id = globalDataEdicion.IdUnidadMedida;
                // console.log(id);
                 $('#<%=IdUnidadMedida.ClientID%>').val(id);
                 $('#<%=txtUmDescripcion.ClientID%>').val(globalDataEdicion.UmDescripcion);
                 $('#<%=txtUnidades.ClientID%>').val(globalDataEdicion.Unidades);
             ...
             $('#gridData').on('click', '#btnEliminar', function (e) {
                 ...
                 var id = data_form.IdUnidadMedida;
                 var isConfirm;
                 //this.preventDefault();
                 e.preventDefault();
                 swal({
                     ...
                     function (isConfirm) {
                         if (isConfirm) {
                             $.ajax({
                                 type: "POST",
                                 data: '{codigo:' + id + '}',
                                 url: "<%= ResolveUrl("Unidad_Medida.aspx/suprData") %>",
                                   contentType: "application/json; charset=utf-8",
                                   dataType: "json",
                                   success: function (response) {
                                       LoadTemp();
                                       $('#<%=IdUnidadMedida.ClientID%>').val('0');
                                       $('#<%=txtUmDescripcion.ClientID%>').val('');
                                       $('#<%=txtUnidades.ClientID%>').val('');
         ...
         quitarClases();
         document.getElementById("UnidadMedida").classList.add("active-page");
