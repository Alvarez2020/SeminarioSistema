<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Pais.aspx.cs" Inherits="SistemaCONNY.CatalogosSistema.Pais" %>
                ...
                <div class="modal-body">
                    ...
                    <div class="panel-body">
                        ...
                        <div hidden class="form-group">
                            <asp:Label Text="ID_PAIS" runat="server" />
                            <asp:TextBox id="IdPais" runat="server" class="form-control" type="text" Text="-1" placeholder="ID_PAIS"></asp:TextBox>
                        </div>
                        <div class="form-group">
                            <asp:Label Text="NOMBRE_PAIS" runat="server" />
                            <asp:TextBox id="txtPais" runat="server" CssClass="form-control" type="text" placeholder="NOMBRE_PAIS"></asp:TextBox>
                            <asp:RequiredFieldValidator ForeColor="red" ValidationGroup="valGuardar" ControlToValidate="txtPais" ID="RequiredFieldValidator1" runat="server" ErrorMessage="Campo es necesario" />
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderScript" runat="server">
     <script>    
         ...
         function LoadTemp() {
             ...
             $.ajax({
                 type: "POST",
                 url: "<%= ResolveUrl("Pais.aspx/CargarDatos") %>",
            ...
            //funcion que permite seleccionar datos de la tabla
             $('#gridData').on('click', '#btnEditar', function () {
                ...
                // console.log(globalDataEdicion);
                 var id = globalDataEdicion.IdPais;
                // console.log(id);
                 $('#<%=IdPais.ClientID%>').val(id);
                 $('#<%=txtPais.ClientID%>').val(globalDataEdicion.NombrePais);
                     ...
                     function (isConfirm) {
                         if (isConfirm) {
                             $.ajax({
                                 type: "POST",
                                 data: '{codigo:' + id + '}',
                                 url: "<%= ResolveUrl("Pais.aspx/suprData") %>",
                                   contentType: "application/json; charset=utf-8",
                                   dataType: "json",
                                   success: function (response) {
                                       LoadTemp();
                                       $('#<%=IdPais.ClientID%>').val('-1');
                                       $('#<%=txtPais.ClientID%>').val('');
         ...
         quitarClases();
         document.getElementById("Pais").classList.add("active-page");
