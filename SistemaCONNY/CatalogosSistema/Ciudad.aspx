<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Ciudad.aspx.cs" Inherits="SistemaCONNY.CatalogosSistema.Ciudad" %>
                ...
                <div class="modal-body">
                    ...
                    <div class="panel-body">
                        ...
                        <div hidden class="form-group">
                            <asp:Label Text="ID_CIUDAD" runat="server" />
                            <asp:TextBox ID="IdCiudad" runat="server" class="form-control" type="text" Text="-1" placeholder="ID_CIUDAD"></asp:TextBox>
                        </div>
                        <div class="form-group">
                            <asp:Label Text="NOMBRE_CIUDAD" runat="server" />
                            <asp:TextBox ID="txtNombre" runat="server" CssClass="form-control" type="text" placeholder="NOMBRE_CIUDAD"></asp:TextBox>
                        ...
                        <div class="form-group">
                            <asp:Label Text="DEPARTAMENTO_CIUDAD" runat="server" />
                            <asp:DropDownList CssClass="form-control" ID="dropDepartamento" runat="server">
                                <asp:ListItem Value="0">Seleccionar</asp:ListItem>
                            </asp:DropDownList>
                            <asp:RequiredFieldValidator ForeColor="red" ValidationGroup="valGuardar" InitialValue="0"  ControlToValidate="dropDepartamento" ID="RequiredFieldValidator2" runat="server" ErrorMessage="Campo es necesario" />
                        </div>
                    </div>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderScript" runat="server">
     <script>    
         ...
         function LoadTemp() {
             ...
             $.ajax({
                 type: "POST",
                 url: "<%= ResolveUrl("Ciudad.aspx/CargarDatos") %>",
            ...
            //funcion que permite seleccionar datos de la tabla
             $('#gridData').on('click', 'btnEditar', function () {
                ...
                // console.log(globalDataEdicion);
                 var id = globalDataEdicion.IdCiudad;
                // console.log(id);
                 $('#<%=IdCiudad.ClientID%>').val(id);
                 $('#<%=txtNombre.ClientID%>').val(globalDataEdicion.NombreCiudad);
                 $('#<%=dropDepartamento.ClientID%>').val(globalDataEdicion.IdDepartamento);
                     ...
                     function (isConfirm) {
                         if (isConfirm) {
                             $.ajax({
                                 type: "POST",
                                 data: '{codigo:' + id + '}',
                                 url: "<%= ResolveUrl("Ciudad.aspx/suprData") %>",
                                   contentType: "application/json; charset=utf-8",
                                   dataType: "json",
                                   success: function (response) {
                                       LoadTemp();
                                       $('#<%=IdCiudad.ClientID%>').val('-1');
                                       $('#<%=txtNombre.ClientID%>').val('');
                                       $('#<%=dropDepartamento.ClientID%>').val('');
         ...
         quitarClases();
         document.getElementById("Ciudad").classList.add("active-page");
